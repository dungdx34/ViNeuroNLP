...
from neuronlp2.nn.attention_aug import AugFeatureHelper, AugBiAAttention
...
class DeepBiAffineTransformNew(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, rnn_mode, hidden_size, num_layers, num_labels, arc_space, type_space,
                 embedd_word=None, embedd_char=None, embedd_pos=None, p_in=0.33, p_out=0.33, p_rnn=(0.33, 0.33), pos=True, activation='elu',
                 use_con_rnn=True,
                 use_gpu=False, no_word=False, use_bert=False):
    ...
    def _get_rnn_output(self, input_word, input_char, input_pos, berts, mask=None):
        # [batch, length, word_dim]
        word = self.word_embed(input_word)
    ...
    def forward(self, input_word, input_char, input_pos, berts, mask=None):
        # output from rnn [batch, length, dim]
        arc, type = self._get_rnn_output(input_word, input_char, input_pos, berts, mask=mask)
        # [batch, length_head, length_child]
        out_arc = self.biaffine(arc[0], arc[1], mask_query=mask, mask_key=mask)
        return out_arc, type

    def loss(self, input_word, input_char, input_pos, heads, types, berts, mask=None):
        # out_arc shape [batch, length_head, length_child]
        out_arc, out_type  = self(input_word, input_char, input_pos, berts, mask=mask)
        # out_type shape [batch, length, type_space]
        type_h, type_c = out_type
    ...
    def decode_local(self, input_word, input_char, input_pos, berts, mask=None, leading_symbolic=0):
        # out_arc shape [batch, length_h, length_c]
        out_arc, out_type = self(input_word, input_char, input_pos, berts, mask=mask)
        batch, max_len, _ = out_arc.size()
        # set diagonal elements to -inf
    ...
    def decode(self, input_word, input_char, input_pos, berts, mask=None, leading_symbolic=0):
        """
        Args:
            input_word: Tensor
                the word input tensor with shape = [batch, length]
...
class StackPtrNetTransform(nn.Module):
    ...
    def _get_encoder_output(self, input_word, input_char, input_pos, berts, mask=None):
        # [batch, length, word_dim]
        word = self.word_embed(input_word)
    ...
    def forward(self, input_word, input_char, input_pos, berts, mask=None, length=None, hx=None):
        raise RuntimeError('Stack Pointer Network does not implement forward')
    ...
    def loss(self, input_word, input_char, input_pos, heads, stacked_heads, children, siblings, stacked_types, berts, mask_e=None, mask_d=None):
        # output from encoder [batch, length_encoder, hidden_size]
        output_enc, hn = self._get_encoder_output(input_word, input_char, input_pos, berts, mask=mask_e)
    ...
    def decode(self, input_word, input_char, input_pos, berts, mask=None, beam=1, leading_symbolic=0):
        # reset noise for decoder
        self.decoder.reset_noise(0)
        ...
        # type_c [batch, length, type_space]
        # hn [num_direction, batch, hidden_size]
        output_enc, hn = self._get_encoder_output(input_word, input_char, input_pos, berts, mask=mask)
        enc_dim = output_enc.size(2)
        device = output_enc.device
        # output size [batch, length_encoder, arc_space]
        arc_c = self.activation(self.arc_c(output_enc))
